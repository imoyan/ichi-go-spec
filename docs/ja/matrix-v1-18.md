...
`SPEC-035` #448 では、単一 state event GET のうち `m.room.name` / empty
`state_key` の代表応答だけを採用対象にします。complete state event type catalog、
custom validation、state_key edge case、full authorization failure matrix は引き続き
広告外です。

Guest boundary は英語 contract が正本です。現在 cover される代表範囲は次の通りです。

- `SPEC-033` #408: `POST /_matrix/client/v3/register?kind=guest` は UIA を要求せず、
  request body の `initial_device_display_name` 以外を guest 作成には使いません。
  代表 vector は `@guest1:example.test`、`token-guest`、`GUEST1` を返します。
- `SPEC-032` #411: その guest token を
  `GET /_matrix/client/v3/account/whoami` に使うと `is_guest: true` を返します。
- `SPEC-033` #426 / #430 / #433: `guest_access_token` による guest-to-user
  upgrade は代表 success path、invalid token / mismatched username の `M_FORBIDDEN`、
  upgrade 後の旧 guest token invalidation、upgraded session の `is_guest: false` までを
  cover します。
- `SPEC-035`: guest token で `POST /_matrix/client/v3/join/{roomIdOrAlias}` する場合、
  room に current `m.room.guest_access` state event がなく
  `content.guest_access = can_join` でないなら `403 M_FORBIDDEN` です。
  `can_join` がある room への direct join success も代表 boundary として cover します。
- `SPEC-035` #438: guest token の non-allowlist state-changing path として
  `POST /_matrix/client/v3/createRoom` を `403 M_GUEST_ACCESS_FORBIDDEN` で拒否する
  代表 fail-closed boundary だけを固定します。

まだ gate 外なのは、room preview event stream、guest-specific rate-limit policy、
representative createRoom rejection を超える guest-specific API allowlist breadth、
...
`SPEC-139` は #386 の Extensible Profiles / Events readiness gate です。
`SPEC-045`、`SPEC-036`、`SPEC-093` は v1.18 の profile / account-data、
send event、sync extension boundary として残し、profile extension、event extension、
capability advertisement、parser-only runtime claim は redacted same-candidate evidence が
揃うまで blocked のままです。
